...
def gerar_dashboard_excel_completo():
    ...
    headers_sondas = ["ID", "Código Sonda", "Equipe Responsável", "Projeto / Frente", "Status Atual"]
    ...
    for r_idx, row in df_sondas.iterrows():
        curr_row = r_idx + 2
        values = [row['id'], row['codigo'], row['equipe'], row['projeto'], row['status']]
    ...
    headers_prod = ["Data", "Sonda", "Furo", "Prof. Inicial (m)", "Prof. Final (m)", "Avanço (m)", "Horas Trab.", "Horas Paradas", "Motivo Parada"]
    ...
    for r_idx, row in df_prod.iterrows():
        ...
        values = [row['data'], row['sonda_codigo'], row['furo_id'], row['prof_inicial'], row['prof_final'], avanco, row['horas_trabalhadas'], row['horas_paradas'], row['motivo_parada']]
        for c_idx, val in enumerate(values, 1):
    ...
    headers_geo = ["Furo", "De (m)", "Até (m)", "Avanço (m)", "Rec. (m)", "Rec. (%)", "RQD (m)", "RQD (%)", "Litologia", "Amostra", "Descrição Geológica", "Observações"]
    ...
    for r_idx, row in df_geo.iterrows():
        curr_row = r_idx + 2
        values = [
            row['furo_id'], row['de_m'], row['ate_m'], row['avanco_m'],
            row['recuperacao_m'], row['recuperacao_pct'], row['rqd_m'], row['rqd_pct'],
            row['litologia'], row['n_amostra'], row['descricao_geologica'], row['observacoes']
        ]
        for c_idx, val in enumerate(values, 1):
...
if opcao == "📊 Dashboard Geral":
    ...
    col1.metric("Sondas Operando", f"{sondas_op}/{sondas_total}")
    col2.metric("Produção Hoje", f"{metros_hoje:.1f} m")
    col3.metric("Produção Acumulada", f"{metros_acumulados:.1f} m")
    col4.metric("Média / Sonda (Hoje)", f"{media_por_sonda:.1f} m")
    col5.metric("Eficiência Operacional", f"{eficiencia:.0f}%")
    ...
    if not df_sondas.empty:
        ...
        st.info("Nenhuma sonda cadastrada no momento. Acesse 'Cadastro de Sondas' para iniciar.")

# CADASTRO DE SONDAS (COM EDICÃO / RENOMEAÇÃO)
    ...
    with col_cad:
        with st.form("form_sonda", clear_on_submit=True):
            ...
            status = st.selectbox("Status Atual", ["Operando", "Parada", "Manutenção"])
    ...
    # RENOMEAR / ATUALIZAR SONDA
    with col_edit:
        if not df_sondas.empty:
            with st.form("form_edit_sonda"):
                st.subheader("✏️ Renomear / Editar Sonda")
                sonda_para_editar = st.selectbox("Selecione a Sonda", df_sondas['codigo'].tolist())
                
                dados_sonda = df_sondas[df_sondas['codigo'] == sonda_para_editar].iloc[0]
                
                novo_codigo = st.text_input("Novo Nome / Código Oficial", value=dados_sonda['codigo'])
                nova_equipe = st.text_input("Equipe", value=dados_sonda['equipe'])
                novo_projeto = st.text_input("Projeto", value=dados_sonda['projeto'])
                novo_status = st.selectbox("Status", ["Operando", "Parada", "Manutenção"], index=["Operando", "Parada", "Manutenção"].index(dados_sonda['status']))
                ...
                if btn_atualizar:
                    ...
                    cursor = conn.cursor()
                    try:
                        cursor.execute("UPDATE sondas SET codigo = ?, equipe = ?, projeto = ?, status = ? WHERE id = ?", (novo_codigo, nova_equipe, novo_projeto, novo_status, int(dados_sonda['id'])))
                        conn.commit()
...
# APONTAMENTO DIÁRIO
    ...
    if not df_prod_full.empty:
        with st.expander("🗑️ Excluir Apontamento Incorreto"):
            opcoes_prod_excluir = df_prod_full.apply(
                lambda r: f"ID {r['id']} | Data: {r['data']} | Sonda: {r['sonda_codigo']} | Furo: {r['furo_id']} ({r['prof_inicial']}m - {r['prof_final']}m)", axis=1
            ).tolist()
            
            apontamento_sel = st.selectbox("Selecione o apontamento que deseja remover:", opcoes_prod_excluir)
            ...
            if st.button("❌ Excluir Apontamento Selecionado", type="secondary"):
                ...
                conn = get_connection()
                cursor = conn.cursor()
                cursor.execute("DELETE FROM producao_diaria WHERE id = ?", (id_prod_excluir,))
                conn.commit()
                conn.close()
    ...
    if not df_sondas.empty:
        with st.form("form_producao", clear_on_submit=True):
            ...
            lista_furos = df_furos['id'].tolist() if not df_furos.empty else ["Sem Furo Cadastrado"]
            furo_sel = col_c.selectbox("Furo", lista_furos)

            col_d, col_e = st.columns(2)
            prof_in = col_d.number_input("Profundidade Inicial (m)", min_value=0.0, step=0.1)
            prof_fim = col_e.number_input("Profundidade Final (m)", min_value=0.0, step=0.1)

            col_f, col_g = st.columns(2)
            hrs_trab = col_f.number_input("Horas Trabalhadas", min_value=0.0, max_value=24.0, value=8.0, step=0.5)
            hrs_par = col_g.number_input("Horas Paradas", min_value=0.0, max_value=24.0, value=0.0, step=0.5)
            motivo_parada = st.text_input("Motivo da Parada")

            btn_reg = st.form_submit_button("Lançar Produção")

            if btn_reg and prof_fim >= prof_in:
                sonda_id = int(df_sondas[df_sondas['codigo'] == sonda_sel]['id'].values[0])
                conn = get_connection()
                cursor = conn.cursor()
                cursor.execute('''
                    INSERT INTO producao_diaria (data, sonda_id, furo_id, prof_inicial, prof_final, horas_trabalhadas, horas_paradas, motivo_parada)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                ''', (str(data_reg), sonda_id, furo_sel, prof_in, prof_fim, hrs_trab, hrs_par, motivo_parada))
                conn.commit()
                conn.close()
...
# CONTROLE DE FUROS
    ...
    if not df_furos_full.empty:
        with st.expander("🗑️ Excluir Furo Cadastrado"):
            furo_para_excluir = st.selectbox("Selecione o Furo que deseja remover:", df_furos_full['id'].tolist())
            st.warning("⚠️ Atenção: Ao excluir um furo, todos os apontamentos de produção e boletins geológicos associados a ele também serão excluídos.")
            
            if st.button("❌ Confirmar Exclusão do Furo", type="secondary"):
                conn = get_connection()
                cursor = conn.cursor()
                
                cursor.execute("DELETE FROM boletim_geologico WHERE furo_id = ?", (furo_para_excluir,))
                cursor.execute("DELETE FROM producao_diaria WHERE furo_id = ?", (furo_para_excluir,))
                cursor.execute("DELETE FROM furos WHERE id = ?", (furo_para_excluir,))
                
                conn.commit()
                conn.close()
                
                st.success(f"Furo {furo_para_excluir} e seus registros associados foram removidos!")
                st.rerun()
    ...
    if not df_sondas.empty:
        with st.form("form_furo", clear_on_submit=True):
            ...
            prof_plan = col3.number_input("Profundidade Planejada (m)", min_value=1.0, step=5.0)
            ...
            if btn_furo and id_furo and sonda_furo:
                sonda_id = int(df_sondas[df_sondas['codigo'] == sonda_furo]['id'].values[0])
                conn = get_connection()
                cursor = conn.cursor()
                try:
                    cursor.execute('''
...
# BOLETIM GEOLÓGICO
    ...
    if df_furos.empty:
        st.warning("Cadastre um furo na aba 'Controle de Furos' antes de registrar dados geológicos.")
        conn.close()
    else:
        ...
        if not df_geo.empty:
            with st.expander("🗑️ Excluir ou Corrigir Registro de Intervalo"):
                opcoes_excluir = df_geo.apply(
                    lambda r: f"ID {r['id']} | {r['de_m']}m - {r['ate_m']}m ({r['litologia']})", axis=1
                ).tolist()
                
                item_selecionado = st.selectbox("Selecione o intervalo que deseja remover:", opcoes_excluir)
                ...
                if st.button("❌ Excluir Intervalo Selecionado", type="secondary"):
                    ...
                    conn = get_connection()
                    cursor = conn.cursor()
                    cursor.execute("DELETE FROM boletim_geologico WHERE id = ?", (id_para_excluir,))
                    conn.commit()
                    conn.close()
                    
                    st.success("Intervalo removido com sucesso!")
                    st.rerun()
        ...
        litologia = col_g3.selectbox("Litologia", [
            "Solo de Alteração", "Basalto Alterado", "Basalto Sano", 
            "Gnaisse", "Quartzito", "Filito", "Itabirito", "Outros"
        ])

        col_g4, col_g5 = st.columns(2)
        rec_m = col_g4.number_input("Recuperação obtida (m)", min_value=0.0, value=1.8, step=0.1)
        rqd_m = col_g5.number_input("Comprimento RQD > 10cm (m)", min_value=0.0, value=1.2, step=0.1)
            ...
            if ate_m > de_m:
                if rec_m <= avanco and rqd_m <= rec_m:
                    ...
                    cursor.execute('''
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                    ''', (furo_selecionado, de_m, ate_m, rec_m, rqd_m, litologia, desc, amostra, obs))
                    conn.commit()
                    conn.close()
                    st.success(f"Intervalo {de_m}m - {ate_m}m registrado com sucesso!")
                    st.rerun()
                else:
                    st.error("Validação falhou: Recuperação deve ser ≤ Avanço e RQD ≤ Recuperação.")
            else:
